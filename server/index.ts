...
async function getSessionUserId(request: BackendRequest): Promise<string | null> {
...
function registerRoutes(app: ExtendedFastifyInstance) {
  ...
  // 获取用户虚拟货币
  app.get('/api/ugc/credit/:userId', async (request: BackendRequest, reply: FastifyReply) => {
    try {
      const { userId } = request.params;
  ...
  app.post(
    '/api/ugc/follow',
    async (
      request: BackendRequest<{ followerId: string; followeeId: string }>,
      reply: FastifyReply,
    ) => {
      try {
  ...
  app.post('/api/ugc/maze/submit', async (request: BackendRequest, reply: FastifyReply) => {
    try {
      const userId = await getSessionUserId(request);
  ...
  app.post('/api/device/reset', async (request: BackendRequest, reply: FastifyReply) => {
    try {
      const userId = await getSessionUserId(request);
  ...
  app.post('/api/vault/key', async (request: BackendRequest, reply: FastifyReply) => {
    try {
      ...
      reply.status(500).send({
        error: error instanceof Error ? error.message : 'Failed to retrieve vault key',
      });
    }
  });
}
