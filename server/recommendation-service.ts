...
export class RecommendationService {
   ...
   */
  async recommendNotesForUser(userId: string, limit: number = 20): Promise<RecommendationItem[]> {
    // 1. 获取用户兴趣标签和分类
    ...
    for (let i = 0; i < allNoteIds.length; i++) {
      ...
      const score = await this.calculateEnhancedRecommendationScore(
        userId,
        note,
        userInterests,
        userBehavior,
   ...
   * 计算多样性探索加成
   */
  private calculateDiversityBonus(noteCategory: string, userCategories: string[]): number {
