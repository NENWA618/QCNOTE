...
export class PushService {
  ...
  constructor() {
    ...
    if (vapidPublicKey && vapidPrivateKey) {
      try {
        ...
        console.warn('⚠ Failed to configure VAPID:', error instanceof Error ? error.message : String(error));
        console.warn('⚠ Web Push notifications will be disabled. Configure VAPID_PUBLIC and VAPID_PRIVATE env vars.');
      }
    } else {
  ...
  async broadcastNotification(
  ...
  ): Promise<{ success: number; failed: number; errors: Array<{ endpoint: string; error: string }> }> {
    ...
    console.log(
      `📢 Broadcast notification sent. Success: ${successCount}, Failed: ${failedCount}`
    );
