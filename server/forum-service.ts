...
export class ForumService {
  ...
  async getUserRole(userId: string): Promise<UserRole> {
    return this.cache.getOrSet(
      `user_role:${userId}`,
      async () => {
        const result = await this.postgres.query('SELECT role FROM user_roles WHERE user_id = $1', [
          userId,
        ]);
    ...
    try {
      ...
      switch (sortBy.toLowerCase()) {
        ...
        case 'trending':
          ...
          query +=
            " AND p.created_at > NOW() - INTERVAL '7 days' ORDER BY (p.like_count + p.reply_count) DESC, p.created_at DESC";
          break;
        case 'newest':
        default:
      ...
      const mockPosts: ForumPost[] = [
        {
          ...
          content:
            '这是 QCNOTE 的社区论坛，您可以在这里与其他用户交流笔记经验、分享使用技巧和获取帮助。',
          categoryId: 'general',
          authorId: 'mock-user',
  ...
  async deletePost(postId: string, userId: string): Promise<boolean> {
    ...
    if (userRole === 'admin' || userRole === 'moderator') {
      ...
      // 普通用户只能删除自己的帖子
      query =
        'UPDATE forum_posts SET is_deleted = true, deleted_at = NOW() WHERE id = $1 AND author_id = $2';
  ...
  async incrementViewCount(postId: string): Promise<void> {
    await this.postgres.query('UPDATE forum_posts SET view_count = view_count + 1 WHERE id = $1', [
      postId,
    ]);
      ...
      return this.cache.getOrSet(
        ...
        async () => {
          const [postsResult, repliesResult, usersResult, categoriesResult] = await Promise.all([
            ...
            this.postgres.query(
              'SELECT COUNT(*) as count FROM forum_replies WHERE is_deleted = false',
            ),
            this.postgres.query(
              'SELECT COUNT(DISTINCT author_id) as count FROM forum_posts WHERE is_deleted = false',
            ),
  ...
  async searchPosts(
    ...
    sqlQuery +=
      ' ORDER BY rank DESC, p.created_at DESC LIMIT $' +
      (params.length + 1) +
      ' OFFSET $' +
      (params.length + 2);
    params.push(limit, offset);
    ...
    // 获取总数
    let countQuery =
      "SELECT COUNT(*) as total FROM forum_posts WHERE is_deleted = false AND search_vector @@ plainto_tsquery('english', $1)";
