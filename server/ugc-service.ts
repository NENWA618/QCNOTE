...
import { UserProfile, LeaderboardEntry, RecommendationItem, HeatmapData } from '../types/ugc-types';
...
export class UGCService {
  ...
  async createUserProfile(userId: string, email: string, username: string): Promise<UserProfile> {
    ...
    await this.db.query(
      `INSERT INTO users(id, email, username, image, provider, bio, joined_at, followers, following, credit, is_public, heatmap, device_fingerprints, current_streak, longest_streak, total_active_days, created_at, updated_at)
   ...
   * 批量货币操作（用于复杂奖励）
   */
  async addCreditBatch(
    operations: Array<{ userId: string; amount: number; reason: string }>,
  ): Promise<void> {
   ...
   * 创意挑战奖励
   */
  async rewardChallengeCompletion(
    userId: string,
    challengeId: string,
    rank: number,
  ): Promise<void> {
  ...
  async unfollowUser(followerId: string, followeeId: string): Promise<void> {
    const result = await this.db.query(
      `DELETE FROM follows WHERE user_id = $1 AND followee_id = $2`,
      [followerId, followeeId],
    );
    if (result.rowCount) {
      await this.db.query(`UPDATE users SET following = GREATEST(following - 1, 0) WHERE id = $1`, [
        followerId,
      ]);
      await this.db.query(`UPDATE users SET followers = GREATEST(followers - 1, 0) WHERE id = $1`, [
        followeeId,
      ]);
      await this.invalidateUserProfileCache(followerId);
      await this.invalidateUserProfileCache(followeeId);
  ...
  async getFollowers(userId: string): Promise<number> {
    ...
    const result = await this.db.query(`SELECT COUNT(*) FROM follows WHERE followee_id = $1`, [
      userId,
    ]);
  ...
  private async addUserCreditHistory(
    userId: string,
    amount: number,
    reason: string,
    newBalance: number,
