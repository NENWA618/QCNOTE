...
describe('Device session token and runtime gating', () => {
  ...
  afterEach(async () => {
    ...
    await Promise.all([
      ...
      deleteDatabase('QCNOTE_NOTES_DB_ALICE').catch(() => {}),
    ]).catch(() => {});
  });
  ...
  it('opens the encrypted notes DB with no secret, via the default server-issued vault key (real production call pattern)', async () => {
    // pages/dashboard.tsx always calls setCurrentUser(userId) with no secret —
    // this reproduces that path end-to-end and is the regression guard for
    // removing the old "silently auto-generate a local key" fallback.
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes('/api/vault/key')) {
        const kekBytes = Uint8Array.from({ length: 32 }, (_, i) => i);
        const kek = btoa(String.fromCharCode(...kekBytes));
        return { ok: true, json: async () => ({ success: true, kek }) } as Response;
      }
    ...
    const storage = new NoteStorage();
    storage['setDeviceSessionToken']('alice', 'device-token');
    await storage.setCurrentUser('alice');
  ...
  it('marks notesDbLocked when the vault key cannot be fetched and no local key exists yet', async () => {
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
      const url = typeof input === 'string' ? input : input.toString();
      if (url.includes('/api/device/session/validate')) {
        return { ok: true, json: async () => ({ success: true }) } as Response;
      }
      // /api/vault/key unreachable (e.g. offline)
      return { ok: false, json: async () => ({ success: false }) } as Response;
    });
