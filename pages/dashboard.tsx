...
const Dashboard: React.FC = () => {
  ...
  const [semanticStatus, setSemanticStatus] = useState<
    'idle' | 'loading-model' | 'searching' | 'ready' | 'error'
  >('idle');
  ...
  // Load semantic search preference once on mount
  useEffect(() => {
    if (typeof window === 'undefined') return;
    setSemanticEnabled(localStorage.getItem(SEMANTIC_SEARCH_STORAGE_KEY) === 'true');
  }, []);
  ...
  const handleToggleSemantic = useCallback(() => {
    setSemanticEnabled((prev) => {
      const next = !prev;
      if (typeof window !== 'undefined') {
        localStorage.setItem(SEMANTIC_SEARCH_STORAGE_KEY, String(next));
      }
      if (!next) {
        setSemanticIds([]);
  ...
  // Semantic search: additive layer on top of the existing keyword search.
  // Debounced so we don't trigger a model inference on every keystroke.
  useEffect(() => {
    if (!semanticEnabled || !search.trim()) {
      setSemanticIds([]);
      if (!semanticEnabled) setSemanticStatus('idle');
      return;
    }
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const ids = await Indexer.getSemanticMatches(search, pool);
        if (!cancelled) {
          setSemanticIds(ids);
        ...
        if (!cancelled) setSemanticStatus('error');
      }
    }, 400);
  ...
  }, [semanticEnabled, search, notes, viewingTrash]);
