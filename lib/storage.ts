...
const LEGACY_WEBDAV_PASSPHRASE = 'qcnote-webdav-default';
const LEGACY_ONEDRIVE_PASSPHRASE = 'qcnote-onedrive-default';
...
const WEBDAV_VAULT_KEY_STORAGE_KEY = 'qcnote:webdav:device-vault-key';
const ONEDRIVE_VAULT_KEY_STORAGE_KEY = 'qcnote:onedrive:device-vault-key';
...
export class NoteStorage {
   ...
   */
  private async fetchVaultKey(token: string, fingerprint: string): Promise<Uint8Array | null> {
    if (typeof window === 'undefined') {
      return null;
    }
    ...
    try {
      ...
      const response = await fetch(new URL('/api/vault/key', baseUrl).toString(), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token, fingerprint }),
      });
  ...
  private async ensureNotesDb(userId: string | null = this.currentUserId): Promise<QCDb | null> {
    ...
    try {
      ...
      if (userId && this.useEncryption && sessionToken && !secret) {
        // The legacy explicit-passphrase path (secret) doesn't use a vault
        // key. Otherwise, validating the session token and fetching the
        // vault key both require the device fingerprint — compute it once.
        const fingerprint = await this.getDeviceFingerprint();
        const validToken = fingerprint
          ? await this.validateDeviceSessionToken(sessionToken)
          : false;
        if (!validToken) {
          this.setDeviceSessionToken(userId, null);
          throw new Error('Invalid or expired device session token');
        }
        kekBytes = fingerprint
          ? ((await this.fetchVaultKey(sessionToken, fingerprint)) ?? undefined)
          : undefined;
      } else if (userId && this.useEncryption && sessionToken && secret) {
      ...
      this.notesDbOpenFailed = true;
      if (e instanceof Error && /required to open|vault key/i.test(e.message)) {
   ...
   */
  private getOrCreateDeviceVaultKeyMaterial(storageKey: string): string {
    ...
    if (!material) {
      ...
      material = this.arrayBufferToBase64(bytes.buffer);
   ...
   */
  private async decryptConfigSecret(
    value: string,
    ...
    const deviceKeyMaterial = this.getOrCreateDeviceVaultKeyMaterial(vaultKeyStorageKey);
    try {
      const decrypted = await this.decryptText(encryptedPart, deviceKeyMaterial);
      return { value: decrypted, needsReencrypt: false };
    } catch {
      const decrypted = await this.decryptText(encryptedPart, legacyPassphrase);
      return { value: decrypted, needsReencrypt: true };
    }
  }

  private async encryptConfigSecret(plain: string, vaultKeyStorageKey: string): Promise<string> {
    const deviceKeyMaterial = this.getOrCreateDeviceVaultKeyMaterial(vaultKeyStorageKey);
    return `encrypted:${await this.encryptText(plain, deviceKeyMaterial)}`;
  }
      ...
      if (config && config.password) {
        try {
          if (config.password.startsWith('encrypted:')) {
            const { value, needsReencrypt } = await this.decryptConfigSecret(
              config.password,
            ...
            if (needsReencrypt) {
              // Migrate off the legacy hardcoded passphrase onto the
              // per-device vault key.
              await this.setWebDAVConfigAsync(config).catch((err) => {
                console.warn('[NoteStorage] Failed to migrate WebDAV password encryption', err);
              });
            }
          }
        } catch (e) {
  ...
  async setWebDAVConfigAsync(config: WebDAVConfig): Promise<boolean> {
    try {
      ...
      if (configToStore.password && !configToStore.password.startsWith('encrypted:')) {
        try {
          configToStore.password = await this.encryptConfigSecret(
    ...
    try {
      ...
      if (config) {
        ...
        if (config.accessToken && config.accessToken.startsWith('encrypted:')) {
          try {
            const result = await this.decryptConfigSecret(
              config.accessToken,
              ONEDRIVE_VAULT_KEY_STORAGE_KEY,
              LEGACY_ONEDRIVE_PASSPHRASE,
            );
            config.accessToken = result.value;
            needsReencrypt = needsReencrypt || result.needsReencrypt;
          } catch (e) {
        ...
        if (config.encryptionKey && config.encryptionKey.startsWith('encrypted:')) {
          try {
            ...
            config.encryptionKey = result.value;
            needsReencrypt = needsReencrypt || result.needsReencrypt;
          } catch (e) {
        ...
        if (needsReencrypt) {
          // Migrate off the legacy hardcoded passphrase onto the per-device
          // vault key.
          await this.setOneDriveConfigAsync(config).catch((err) => {
            console.warn('[NoteStorage] Failed to migrate OneDrive secret encryption', err);
          });
        }
      }
      return config;
  ...
  async setOneDriveConfigAsync(config: OneDriveConfig): Promise<boolean> {
    try {
      ...
      if (configToStore.accessToken && !configToStore.accessToken.startsWith('encrypted:')) {
        try {
          configToStore.accessToken = await this.encryptConfigSecret(
      ...
      if (configToStore.encryptionKey && !configToStore.encryptionKey.startsWith('encrypted:')) {
        try {
          configToStore.encryptionKey = await this.encryptConfigSecret(
  ...
  async setDataAsync(notes: NoteItem[]): Promise<boolean> {
    ...
    try {
      ...
      if (this.notesDbLocked) {
        // The vault key couldn't be fetched (offline, device not verified,
        // or a server-side vault misconfiguration) and there's no
        // already-migrated legacy local key either. storeNotesByKey's
        // fallback below only works for the opt-in explicit-passphrase
        // scheme (currentEncryptionKey) — vault-scheme accounts never set
        // that, so calling it here would throw a misleading error instead
        // of the "device locked" state callers should be told about.
        console.warn(
          '[NoteStorage] notesDb 已锁定（无法获取加密密钥），拒绝回退到旧版明文/密码存储',
        );
        return false;
      }
      ...
      reader.onload = async (e: ProgressEvent<FileReader>) => {
        try {
          ...
          if (!success) {
            reject(
              this.notesDbLocked
                ? '导入失败：当前设备未解锁（无法获取加密密钥）。请检查网络连接，或重新验证/注册当前设备后重试。'
                : '导入失败：写入存储失败',
            );
            return;
          }
