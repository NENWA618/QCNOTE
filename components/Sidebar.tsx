...
interface Props {
  ...
  semanticStatus?: 'idle' | 'loading-model' | 'searching' | 'ready' | 'error';
            ...
            {filtersOpen && (
              <div className="mt-3 space-y-3">
                ...
                <div>
                  ...
                  />
                  {onToggleSemantic && (
                    <label className="mt-2 flex items-center gap-2 text-xs text-gray-600 dark:text-dark-text-secondary cursor-pointer">
                      <input
                        type="checkbox"
                  ...
                  {semanticEnabled && search && semanticStatus !== 'idle' && (
                    <p className="mt-1 text-xs text-text-light dark:text-dark-text-secondary">
                      {semanticStatus === 'loading-model' &&
                        `下载模型中 ${semanticProgress.toFixed(0)}%`}
                      {semanticStatus === 'searching' && '正在语义搜索...'}
                      {semanticStatus === 'ready' && '语义搜索就绪'}
                      {semanticStatus === 'error' && '语义搜索出错，已回退到普通搜索'}
                    </p>
                  )}
                </div>
              </div>
