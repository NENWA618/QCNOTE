...
const Footer: React.FC<FooterProps> = ({
  ...
  if (layout === 'full' || layout === 'compact') {
    const defaultLinks = [
      { label: '← 返回首页', href: '/' },
      { label: '✍️ 前往笔记 →', href: '/dashboard' },
    ];

    const links = customLinks || defaultLinks;

    return (
      <footer role="contentinfo" aria-label="Site footer" className="footer-panel py-12 px-6">
        <div className="max-w-6xl mx-auto">
          {/* 三列网格部分 */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            ...
            {showAbout && (
              <div>
                <h4 className="footer-section-title">📝 关于 QCNOTE</h4>
