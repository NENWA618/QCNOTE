...
const NoteEditor: React.FC<NoteEditorProps> = ({
  ...
  const unresolvedForwardLinks = (localNote.links || [])
    .filter((title) => !forwardLinks.some((note) => note.title === title));
                  ...
                  {forwardLinks.length > 0 ? (
                    <ul className="space-y-2 text-sm">
                      ...
                      {unresolvedForwardLinks.length > 0 && (
                        <li className="text-gray-500">未匹配笔记: {unresolvedForwardLinks.join('、')}</li>
                      )}
                    </ul>
                  ) : (
