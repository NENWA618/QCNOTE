...
export default function ForumPostDetail({ post, replies, totalReplies }: ForumPostDetailProps) {
  ...
  const currentReplyTarget = replyingTo && replyingTo !== 'main'
    ? replies.find((reply) => reply.id === replyingTo)
    : null;
    ...
    return sortedReplies.map((reply, index) => {
      ...
      return (
        <div key={reply.id} className={`flex ${alignClass}`}>
          <div className={`max-w-[85%] rounded-[2rem] border border-slate-200 px-5 py-4 shadow-sm ${bubbleClass} dark:border-gray-700`}>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-gray-400">{formatDate(reply.createdAt)}</p>
                </div>
              </div>
              <button
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-500 dark:text-gray-400">{post.authorName}</p>
