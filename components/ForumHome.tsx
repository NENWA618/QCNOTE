        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-2xl backdrop-blur-sm dark:border-slate-700 dark:bg-slate-950/80">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue-600">社区论坛</p>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-900">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">帖子</p>
              <p className="mt-3 text-3xl font-bold text-slate-900 dark:text-white">{stats.totalPosts}</p>
            </div>
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-900">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">回复</p>
              <p className="mt-3 text-3xl font-bold text-slate-900 dark:text-white">{stats.totalReplies}</p>
            </div>
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-900">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">用户</p>
              <p className="mt-3 text-3xl font-bold text-slate-900 dark:text-white">{stats.totalUsers}</p>
            </div>
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-900">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">分类</p>
              <p className="mt-3 text-3xl font-bold text-slate-900 dark:text-white">{stats.totalCategories ?? categories.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 space-y-6">
          <div className="rounded-[2rem] border border-slate-200 bg-white/90 p-5 shadow-xl backdrop-blur-sm dark:border-slate-700 dark:bg-slate-950/85">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="text-sm text-slate-500 dark:text-slate-400">当前分类：{selectedCategory ? (categories.find((item) => item.id === selectedCategory || item.categoryId === selectedCategory)?.name ?? '自定义') : '全部'}</div>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white/90 p-10 text-center shadow-xl dark:border-slate-700 dark:bg-slate-950/85">
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">试试更宽泛的关键词或切换分类。</p>
              {session?.user && (
                <Link
                  href="/forum-create"
                  ...
                  <div className="relative space-y-5">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      ...
                      <div className="flex flex-col gap-3 text-slate-500 dark:text-slate-400">
                        <div className="rounded-[1.75rem] border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">发布时间</p>
                          <p className="mt-2 text-base font-semibold text-slate-900 dark:text-white">{formatDate(post.createdAt)}</p>
                        </div>
                        <div className="rounded-[1.75rem] border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">作者</p>
                          <p className="mt-2 text-base font-semibold text-slate-900 dark:text-white">{post.authorName}</p>
                        </div>
                        <div className="rounded-[1.75rem] border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">身份</p>
                          <p className="mt-2 text-base font-semibold text-slate-900 dark:text-white">
                            {post.authorRole === 'admin' ? '管理员' : post.authorRole === 'moderator' ? '版主' : '用户'}
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-wrap items-center justify-between gap-3 rounded-[1.75rem] border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                      <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500 dark:text-slate-400">
                        <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-slate-700 dark:bg-slate-800 dark:text-slate-200">💬 {post.replyCount}</span>
                        <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-slate-700 dark:bg-slate-800 dark:text-slate-200">👁 {post.viewCount}</span>
                        <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-slate-700 dark:bg-slate-800 dark:text-slate-200">👍 {post.likeCount}</span>
                      </div>
                      <Link
