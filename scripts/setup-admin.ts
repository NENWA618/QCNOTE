...
async function setupAdmin() {
  ...
  try {
    ...
    // 首先尝试通过ID查找用户
    let userResult = await pool.query('SELECT id, name, email, username FROM users WHERE id = $1', [
      identifier,
    ]);
    ...
    if (userResult.rows.length === 0) {
      ...
      if (userResult.rows.length === 0) {
        ...
        await pool.query(
          `INSERT INTO users(id, email, username, image, provider, bio, joined_at, is_public, heatmap, current_streak, longest_streak, total_active_days, created_at, updated_at)
        ...
        // 重新查询用户
        userResult = await pool.query('SELECT id, name, email, username FROM users WHERE id = $1', [
          identifier,
        ]);
      }
    }
