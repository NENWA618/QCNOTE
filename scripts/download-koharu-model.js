  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(url, (res) => {
      if (res.statusCode !== 200) {
        reject(new Error(`Download failed: ${url} status code ${res.statusCode}`));
        return;
      }
      res.pipe(file);
      file.on('finish', () => {
...
async function run() {
  const modelDir = path.join(__dirname, '../public/live2d/koharu');
  const baseUrl = 'https://raw.githubusercontent.com/xiazeyu/live2d-widget-models/master/packages/live2d-widget-model-koharu/assets/';
