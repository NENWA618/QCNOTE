...
完成部署后，访问前端应用：
1. 确保页面已加载 `jquery`、`jquery-ui`、`waifu-tips` 及 `waifu.js` 脚本。
2. 页面右下角出现看板娘并出现欢迎消息。
3. 点击看板娘图标可显示/隐藏，看板娘会触发配置的事件（语言、表情、健康提醒等）。
4. 若看板娘未出现，请检查浏览器控制台有没有脚本错误，或来自 `waifu.js` 的加载路径。

---

## 说明

- 本项目已删除旧提醒与推送系统，现仅保留 Live2D 插件相关逻辑。
- 若需要启用自定义推送，请在 `waifu.js` 中实现对应语义和通知逻辑。
