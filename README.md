...
- **🔐 DSL 加密存储**: 笔记主体字段（标题和内容）使用 QCNOTE DSL 字段级 AES-GCM 加密存储于本地
- **👤 账号隔离**: 不同登录账号的数据按命名空间隔离存储，避免账号间数据混淆
- **☁️ 云端同步**: 支持 WebDAV 和 OneDrive 跨设备同步
- **🔄 冲突解决**: 自动检测并帮助解决同步冲突
- **📦 导入导出**: 支持多种格式的数据导入导出
...
- **QCNOTE DSL**: 客户端加密存储运行时，支持字段级 AES-GCM 加密和账号隔离存储
- **Lunr.js**: 全文搜索引擎
- **向量搜索**: 自定义语义搜索算法
- **情感分析**: Sentiment.js
